...
void merge(vector<Bid>& bids, int l, int m, int r) {
    int n1 = m - l + 1; 
    int n2 = r - m;
    vector<Bid> L(n1);
    vector<Bid> R(n2);
    ...
    while (i < n1 && j < n2) {
        if (L[i].amount > R[j].amount || 
            (L[i].amount == R[j].amount && L[i].timestamp <= R[j].timestamp)) { 
            bids[k++] = L[i++];
        } else {
            bids[k++] = R[j++];
        }
...
void mergeSortAuction(vector<Bid>& bids, int l, int r) {
    if (l >= r) 
        return; 
    int m = l + (r - l) / 2;
    mergeSortAuction(bids, l, m);
    mergeSortAuction(bids, m + 1, r);
    merge(bids, l, m, r);
}
...
void sortAuctionBids(vector<Bid>& bids) {
    ...
    mergeSortAuction(bids, 0, bids.size() - 1);
}
...
void sortStudentGrades(vector<int>& grades) {
    // TODO: Implement the appropriate sorting algorithm
    int maxgrade = 100;
    int bucketcount = maxgrade + 1; 
    vector<vector<int>> buckets(bucketcount);

    for (int i = 0; i < grades.size(); i++) {
        buckets[grades[i]].push_back(grades[i]);
    }
    int gradeindex = 0;
    
    for (int i = 0; i < bucketcount; ++i) {
        for (int grade : buckets[i]) {

...
private:
    ...
    long long hashContent(const string& content) {
        ...
        for (int i = 0; i < (int)content.length(); i++) {
            ...
            hashValue = (hashValue + c) % MOD;
        }

        return (long long)hashValue;
    }
...
public:
    ...
    int findDuplicates(const vector<File>& files) {
        ...
        for (int i = 0; i < (int)files.size(); i++) {
            long long h = hashContent(files[i].content);
            groups[h].push_back(i);
        }

        int duplicateGroupCount = 0;

        for (pair<const long long, vector<int>> entry : groups) {
            vector<int> idxs = entry.second;
            if ((int)idxs.size() <= 1) {
                continue;
            }
            vector<vector<int>> exactGroups;

            for (int a = 0; a < (int)idxs.size(); a++) {
                int fileIndex = idxs[a];
                bool placed = false;

                for (int g = 0; g < (int)exactGroups.size(); g++) {
                    int representativeIndex = exactGroups[g][0];
                    if (files[fileIndex].content == files[representativeIndex].content) {
                        exactGroups[g].push_back(fileIndex);
                        placed = true;
                        break;
                    }
                }
                ...
                if (!placed) {
                    vector<int> newGroup;
                    newGroup.push_back(fileIndex);
                    exactGroups.push_back(newGroup);
                }
            }

            for (int g = 0; g < (int)exactGroups.size(); g++) {
                if ((int)exactGroups[g].size() > 1) {
                    duplicateGroupCount++;
                    cout << "Duplicate Group " << duplicateGroupCount << ": ";
                    for (int k = 0; k < (int)exactGroups[g].size(); k++) {
                        int fileIndex = exactGroups[g][k];
                        cout << files[fileIndex].name;
                        if (k < (int)exactGroups[g].size() - 1) {
                            cout << ", ";
                        }
                    }
                    cout << endl;
